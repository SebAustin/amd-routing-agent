...
- `policies/default.yaml` — initial policy, pre-tuning.
...
1. **Dry-run coverage** (`default-dry-run.json`): Tier-0 solved 61/200 (30.5%) tasks pre-tuning,
   65/200 (32.5%) post-tuning — both clear SC3's ≥30% zero-token target.
2. **Live default** (`default-live.json`): 91.5% accuracy pre-classifier/prompt/registry fixes.
3. **Live tuned** (`tuned-live.json`): 99.5% accuracy after fixes (see commit history / report
   header comments for the full list). One residual failure: `summarization-003` — the model's
   correct paraphrase drops the literal keyword "declin[e/ing]" that `contains_all` requires;
   diminishing returns to chase further on a 5-task category.
